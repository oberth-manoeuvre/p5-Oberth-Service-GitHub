...
use SeeLucid::Service::GitHub::Repo;
...
subtest "Throws exceptions for bad URIs" => sub {
	for my $repo (@$data) {
		lives_ok { SeeLucid::Service::GitHub::Repo->new( uri => $repo->{uri} ) } 'valid GitHub URI';
	}
};

subtest "Correct name extraction" => sub {
	for my $repo (@$data) {
		my $gh = SeeLucid::Service::GitHub::Repo->new( uri => $repo->{uri} );
...
subtest 'HTTPS web URI' => sub {
	my $gr = SeeLucid::Service::GitHub::Repo->new( uri => 'git@github.com:test/repo.git' );
	is( $gr->github_https_web_uri, 'https://github.com/test/repo', 'correct HTTPS web URI for GitHub repo');
};

subtest 'Pithub data' => sub {
	my $gr = SeeLucid::Service::GitHub::Repo->new( namespace => 'SeeLucid', name => 'p5-Project-Manager' );
