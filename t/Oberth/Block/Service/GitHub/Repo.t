...
subtest "Throws exceptions for bad URIs" => sub {
	for my $repo (@$data) {
		lives_ok { Oberth::Block::Service::GitHub::Repo->new( uri => $repo->{uri} ) } 'valid GitHub URI';
	}
};

subtest "Correct name extraction" => sub {
	for my $repo (@$data) {
		my $gh = Oberth::Block::Service::GitHub::Repo->new( uri => $repo->{uri} );
...
subtest 'HTTPS web URI' => sub {
	my $gr = Oberth::Block::Service::GitHub::Repo->new( uri => 'git@github.com:test/repo.git' );
	is( $gr->github_https_web_uri, 'https://github.com/test/repo', 'correct HTTPS web URI for GitHub repo');
};

subtest 'Pithub data' => sub {
	my $gr = Oberth::Block::Service::GitHub::Repo->new( namespace => 'oberth-manoeuvre', name => 'p5-Oberth-Manoeuvre' );
